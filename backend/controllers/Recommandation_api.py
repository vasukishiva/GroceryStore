...
class RecommendationAPI(Resource):
    ...
    def get(self):
        ...
        # 2️⃣ If no orders → fallback to cart
        if not product_ids:
            cart_items = CartItems.query.filter_by(user_id=user.id).all()
            for item in cart_items:
                product_ids.add(item.product_id)

        # 3️⃣ If still empty → return popular products
        if not product_ids:
            products = Products.query.limit(5).all()
        else:
            # 4️⃣ Get categories of these products
            categories = set()
            ...
            # 5️⃣ Recommend products from same categories
